...
rng_key, k_qr, k_eigs = jax.random.split(rng_key, 3)
Q, _ = jax.vmap(jnp.linalg.qr)(jax.random.normal(k_qr, (n_mixes, dim, dim)))
eigvals = jnp.exp(jax.random.uniform(k_eigs, (n_mixes, dim), minval=jnp.log(0.1), maxval=jnp.log(2.0))) * log_var_scaling
covariances = jnp.einsum("...ij,...j,...kj->...ik", Q, eigvals, Q)
...
com = distrax.MultivariateNormalFullCovariance(loc=means, covariance_matrix=covariances)
...
true_logZ = prior.log_prob(jnp.ones(10))
...
smc_ss_states, smc_ss_res = run_ss_sequential_mc(
    sampling_key, mog.log_prob, prior.log_prob, num_mcmc_steps=dim, target_ess=0.9, initial_samples=initial_samples
)
...
ns_states, ns_res = run_nested_sampling(
    sampling_key, mog.log_prob, prior.log_prob, num_mcmc_steps=dim, initial_samples=initial_samples, num_delete=100
)

rng_key, sampling_key = jax.random.split(rng_key)
ns_mc_states, ns_mc_res = run_rw_nested_sampling(
    sampling_key, mog.log_prob, prior.log_prob, num_mcmc_steps=5 * dim, initial_samples=initial_samples, num_delete=100
)

rng_key, sampling_key = jax.random.split(rng_key)
smc_states, smc_res = run_rw_sequential_mc(
    sampling_key, mog.log_prob, prior.log_prob, num_mcmc_steps=5 * dim, target_ess=0.9, initial_samples=initial_samples
)
...
ns_samples = ns_samples.position
ns_mc_samples = ns_mc_samples.position
hmc_samples = hmc_states.position[::10]
ss_samples = ss_states.position[::10]
rng_key, true_samples_key = jax.random.split(rng_key)
true_samples = mog.sample(seed=true_samples_key, sample_shape=(3000,))
...
plot_comparison_grid(
    ...
    output_prefix="plots/mixture_gaussians_10d",
)
...
print(f"True logZ: {true_logZ:.2f}")
